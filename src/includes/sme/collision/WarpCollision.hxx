...
public:
  ...
  TCollisionLink(SearchMode mode)
      : mColTriangle(nullptr), mTargetID(NullID), mHomeID(NullID),
        mSearchMode(mode) {}
  TCollisionLink(TBGCheckData *tri, u8 targetID, u8 homeID, SearchMode mode)
      : mColTriangle(tri), mTargetID(targetID), mHomeID(homeID),
        mSearchMode(mode) {}
  ...
  void setThisColTriangle(TBGCheckData *colTriangle) {
    mColTriangle = colTriangle;
  }
...
public:
  TWarpCollisionList(size_t size) : mUsedSize(0), mMaxSize(size) {
    mColList =
        reinterpret_cast<Class::TCollisionLink *>(JKRHeap::sCurrentHeap->alloc(
            sizeof(Class::TCollisionLink) * size, 4));
  };
