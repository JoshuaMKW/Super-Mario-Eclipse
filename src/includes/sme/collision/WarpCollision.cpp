...
TCollisionLink::SearchMode
TCollisionLink::getSearchModeFrom(TBGCheckData *colTriangle) {
...
TCollisionLink::WarpType
TCollisionLink::getWarpTypeFrom(TBGCheckData *colTriangle) {
  switch (colTriangle->mCollisionType) {
...
bool TCollisionLink::isTargetOf(TBGCheckData *other) const {
  ...
  if (getSearchModeFrom(other) == SearchMode::BOTH ||
      getSearchMode() == SearchMode::BOTH) {
    return true;
    ...
    return (minDist * minDist) <
           GetSqrDistBetweenColTriangles(other, getThisColTriangle());
  }
...
bool TCollisionLink::isTargeting(TBGCheckData *other) const {
  ...
  if (getSearchMode() == SearchMode::BOTH ||
      getSearchModeFrom(other) == SearchMode::BOTH) {
    return true;
    ...
    return (minDist * minDist) <
           GetSqrDistBetweenColTriangles(getThisColTriangle(), other);
  }
...
void TWarpCollisionList::addLink(TBGCheckData *a, TBGCheckData *b) {
  TCollisionLink link(a, static_cast<u8>(b->mValue4),
                      static_cast<u8>(a->mValue4),
...
void TWarpCollisionList::addLink(TCollisionLink &link) {
  if (mUsedSize >= mMaxSize) {
    SME_DEBUG_LOG("TWarpCollision::addLink(): Collision list is full!\n");
    return;
  }
  SME_DEBUG_LOG(
      "TWarpCollision::addLink(): (%d) Added link of type %d at 0x%X\n",
...
TBGCheckData *
TWarpCollisionList::resolveCollisionWarp(TBGCheckData *colTriangle) {
...
TBGCheckData *TWarpCollisionList::getNearestTarget(TBGCheckData *colTriangle) {
  ...
  TVectorTriangle colVector(colTriangle->mVertexA, colTriangle->mVertexB,
  ...
  case TCollisionLink::SearchMode::DISTANCE: {
    ...
    for (u32 i = 0; i < size(); ++i) {
      ...
      if (!colLink.isValidDest())
        continue;
      if (GetSqrDistBetweenColTriangles(colLink.getThisColTriangle(),
                                        colTriangle) > (minDist * minDist) &&
          colLink.getThisColTriangle() != colTriangle) {
  ...
  case TCollisionLink::SearchMode::BOTH: {
    ...
    for (u32 i = 0; i < size(); ++i) {
      if (!mColList[i].isValidDest())
        continue;
      if (mColList[i].getHomeID() ==
              TCollisionLink::getTargetIDFrom(colTriangle) &&
